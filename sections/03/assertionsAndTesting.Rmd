...
You can also write you own assertions with custom error messages.  This is done using the 
`on_failure()` function. Below is an example of how this works: 
...
Suppose we want to write a function `calculator(x, y, method)` that takes in two numbers `x` and `y` as well as a string `method` indicating whether to perform addition, subtraction, multiplication, or division.  This function should return a numeric value.  

1. Write a test that checks whether `calculator` returns an error when `x` and `y` are not numeric and when `method` is not in the expected set of operations (i.e. addition, subtraction, multiplication, and division).  Save these tests in a file called `tests-calculator.R`.  

2. Start writing your `calculator` function with assertions that produces errors if `x` or `y` are not numeric or when `method` is not in the expected set of operations.  You can choose what you expect the user to call each operation in the function.  Save this function as `calculator.R`

3. Use the `test_files("tests-calculator.R")` to see if your function is operating as you hope. 
...
4. Write a test that checks whether the addition piece of your calculator produces the correct results with the following input: 
  a. `x = 1` and `y = 9`
  b.  `x = 100`, `y = -5`
  c. `calculator` returns a numeric value
Save these tests in a file called `tests-calculator.R`.  

5. Add the addition functionality to `calculator()` and call `test_files("tests-calculator.R")` again.  

6. Continue iterating through this process for substraction, multiplication, and division.  Make sure your function elegantly handles division when the demoninator is 0 with an assertion and with testing.
