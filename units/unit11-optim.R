...
plot(xs, fp(xs), type = 'l', xlab = 'x', ylab = "f'(x)", main ='converges quickly')
lines(xs, fpp(xs), lty = 3)
legend('topleft', bty = 'n', lty = c(1,3), legend = c("f'(x)", 'f"(x)'))
points(xvals, fp(xvals), pch = as.character(1:length(xvals)), col = 'red', cex = 1.5)
...
legend('bottomright', legend = c("f(x)"), bty = 'n')
points(xvals, f(xvals), pch = as.character(1:length(xvals)), col = 'red', cex = 1.5)
